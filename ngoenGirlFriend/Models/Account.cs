...
namespace ngoenGirlFriend.Models
{
    public class Account
    {
        ...
        {
            string query = String.Format("SELECT fullname,imageurl,roleId,userid,password FROM accountUser WHERE username = '{0}' AND password ='{1}'", username, password);
        ...
        public void changeinfor(string fullname, string phone, string Birthdate, string imageurl, string email, string uWardid, string uDistrictid, string uProvinceid, string id)
        {
            string query = "UPDATE accountUser SET fullname=N'" + fullname + "', phone='" + phone + "',birthdate='" + Birthdate + "',imageurl='" + imageurl + "',email='" + email + "',uWardid=" + uWardid + ",uDistrictid=" + uDistrictid + ",uProvinceid=" + uProvinceid + " WHERE userid=" + id;
            sql.excuteNonQuery(query);
        }
        public void changeinfor1(string fullname, string phone, string Birthdate, string email, string uWardid, string uDistrictid, string uProvinceid, string id)
        {
            string query = "UPDATE accountUser SET fullname=N'" + fullname + "', phone='" + phone + "',birthdate='" + Birthdate + "',email='" + email + "',uWardid=" + uWardid + ",uDistrictid=" + uDistrictid + ",uProvinceid=" + uProvinceid + " WHERE userid=" + id;
            sql.excuteNonQuery(query);
        }
        public void changepassword(string password, string ID)
        {
            string query = "UPDATE accountUser SET password='" + password + "'Where userid=" + ID;
            sql.excuteNonQuery(query);
        }
    }
}